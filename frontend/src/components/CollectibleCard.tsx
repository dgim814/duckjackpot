...
export function CollectibleCard({ card, raffleId }: CollectibleCardProps) {
  const { t } = useI18n()
  ...
  return (
    <article className="card-foil relative overflow-hidden rounded-[28px] border border-amber-400/35 bg-black shadow-[0_20px_50px_rgba(255,107,0,0.22)]">
      ...
      <div className="absolute inset-x-0 top-0 z-20 flex items-center justify-between px-4 py-3">
        <span className="rounded-full bg-black/55 px-2.5 py-1 text-[10px] font-extrabold tracking-[0.14em] text-amber-300 backdrop-blur-sm">
          {t(RAFFLE_TITLE_KEY[raffle.id])}
        </span>
        <span className="rounded-full bg-black/55 px-2.5 py-1 font-display text-[11px] font-bold text-amber-200 backdrop-blur-sm">
          #{serial}
        </span>
      </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black via-black/80 to-transparent px-4 pb-4 pt-16">
        <p className="font-display text-lg font-extrabold tracking-wide text-amber-200">
          DUCK JACKPOT
        </p>
